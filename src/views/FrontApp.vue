<template>
    <v-container class="home app-page">
        ...
        <div class="buttons d-flex flex-column">
            ...
            <v-btn @click="emptyDb" :disabled="dbStatus ==='empty' || dbStatus ==='init'">
        ...
        >
            ...
            >
                <template v-slot:header>
                    <div>{{attr}} </div><small class="text-center" v-if="wordArr.length > 999">(can't fetch more then 1000 words) </small><small class="text-right mr-3">total: {{wordArr.length}} items</small>
                </template>
                <v-card>
...
<script>
    export default {
        ...
        computed:{
            wordsObj(){return this.$store.getters['wordsObj']},
            dbStatus(){return this.$store.getters['dbStatus']},
            words(){return this.$store.getters['words']},
        }
    }
</script>
